...
## 🍽️ O Cardápio Gastronômico de Planos & Assinaturas (`/planos`)

A seção de planos e aquisição de tokens do portal discente é gamificada no formato de um **Cardápio de Restaurante / Bistrô Premium**, tornando a experiência de compra imersiva e prazerosa:

### 1. Pratos Principais do Menu
- **🥖 Menu Degustação Acadêmica (STARTER):** Entrada cortesia da casa com 100 tokens mensais e funcionalidades fundamentais (linha do tempo, 5 memórias/mês, busca pública de editais).
- **🍽️ Banquete do Pesquisador - Chef's Special (MAIN - R$ 29,90/mês):** Prato assinatura com 1.500 tokens mensais, IA Copilot ilimitada, radar ativo de editais com afinidade personalizada, memórias com áudio e transcrição, validador de certificados e exportação de portfólio em PDF.
- **🍷 Reserva Especial Grand Cru (GRAND_CRU - R$ 69,90/mês):** Harmonização completa para laboratórios e pesquisadores de ponta com 5.000 tokens de modelos avançados, diagnósticos de gap analysis, simulações Lattes/CNPq e badge exclusivo "Membro Grand Cru".
- **🍰 Petit Four de Tokens (SIDE - R$ 9,90):** Sobremesa rápida a la carte com +500 tokens imediatos sem expiração para momentos de pico.

### 2. A Comanda (Carrinho & Regra de Acúmulo / Stacking)
- O estudante pode adicionar múltiplos pratos ou porções à sua comanda.
- **Regra de Acúmulo com Feedback em Tempo Real:** Se o usuário já possui as funcionalidades ativas de um prato principal ou adiciona mais de uma unidade, a interface emite alerta visual instantâneo:
  > *"Você já desbloqueou as funcionalidades deste item. Adicionar mais uma unidade irá apenas somar +X tokens à sua conta."*
- Os tokens acumulam integralmente no saldo da carteira do estudante sem desperdício.

### 3. A Gorjeta do Chef (Upsell Voluntário de Tokens)
- No momento do fechamento da comanda, o discente pode optar por incluir uma gorjeta para a manutenção e sustentabilidade da infraestrutura da plataforma.
- **Bonificação:** Cada R$ 1,00 de gorjeta concedido é retribuído diretamente com **60 tokens de IA adicionais**, creditados imediatamente na carteira.
- Finalizada a comanda, é emitido um **Recibo Gastronômico Digital** com ID de comprovação e notificação in-app de confirmação.
